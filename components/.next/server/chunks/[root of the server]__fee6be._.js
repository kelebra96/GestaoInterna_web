...
"[project]/app/api/mensagens/users/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    try {
        const auth = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$helpers$2f$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getAuthFromRequest"])(req);
        if (!auth) {
        ...
        const { data, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2d$admin$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseAdmin"].from('users').select('id, display_name, email, role, last_seen, active, company_id').eq('active', true).order('display_name', {
            ascending: true
        });
        if (error) {
        ...
        const users = (data || []).filter((user)=>user.id !== currentUserId) // Excluir usuário atual
        .map((user)=>{
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__fee6be._.js.map